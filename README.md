...
Both check the archive against the published SHA-256 sums and install nothing if
that check fails. `LETEO_INSTALL_DIR` chooses where the binary goes,
`LETEO_VERSION` pins a version, and `LETEO_BASE_URL` points at a mirror. They
start working with the first tagged release; until then, build from source:

```bash
cargo install --git https://github.com/asanabrial/leteo
```

Then wire up your agent — `leteo setup` with no arguments walks through it:

```powershell
leteo setup
leteo setup claude-code --hooks
leteo setup opencode --dry-run
leteo setup codex --instructions
leteo uninstall
```

`--dry-run` shows the change without making it, and every adapter leaves the
rest of the configuration file alone.
