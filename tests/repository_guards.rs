...
///
/// So the first drifts. Two reviews of the change that added the figure found
/// the same shape of defect three times over — a median hard-copied beside the
/// list it came from, a run count spelled out beside the list that determines
/// it, an off-protocol caveat carried in the picture and dropped from the text
/// that replaces the picture. Making the `alt` attribute *be* the figure's own
/// `aria-label` removes the second copy; this is what keeps it removed.
#[test]
fn the_figure_describes_itself_the_same_way_twice() {
    ...
    assert_eq!(
        ...
        "README.md's alt text for the token figure has drifted from the figure's own \
         aria-label. The aria-label is generated by tools/chart/tokens.py from the runs; the \
         alt is not, so it is the one that goes stale. Copy it across rather than editing it:\n\
         \n  aria-label: {aria}\n\n  alt:        {alt}\n"
    );
}
